...
<?php 
    ...
    if($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        if(isset($_POST['add']) && isset($_POST['account']) && isset($_POST['amount']) & isset($_POST['pin'])) {
            ...
            $result = $conn->query('SELECT AccountNumber FROM accounts WHERE Type="admin" LIMIT 1;');
            if(!$result->num_rows > 0) {
                http_response_code(500);
                exit;
            }
            $row = $result->fetch_assoc();
            $debit_account = $row['AccountNumber'];
            ...
            if($result->num_rows > 0 && $destination_result->num_rows > 0) {
                ...
                if($tranfer_source_balance > $amount_to_transfer) {
                    ...
                    $history->set_body('You revieve '.$amount_to_transfer.' USD from '.ACCOUNT['AccountNumber'].', on '.date('l, d M, Y').', at '.date('H:i'));
                    $history->save();
